...
class TranscriptionWorker:
    ...
    def __init__(
        ...
        # 录音 WAV 持久化开关（默认关闭：识别用临时文件，用完即删，不在 logs/ 留档）
        self._save_recordings = bool(audio_cfg.get("save_recordings", False))
    ...
    def _apply_gain(self, arr: np.ndarray) -> np.ndarray:
        """对 int16 音频应用增益并削波保护，避免内置麦克风音量过低。"""
        try:
            scaled = arr.astype(np.float32) * self.gain
            return np.clip(scaled, -32768.0, 32767.0).astype(np.int16)
        except Exception as exc:
            logger.warning("增益处理失败，使用原始音频: %s", exc)
            return arr
    ...
    def _write_recent_wav(self, samples: np.ndarray) -> None:
        """将最近一次录音保存为 recent.wav（仅调试用途，受 save_recordings 开关控制）。"""
        import wave

        if not self._save_recordings:
            self.last_segment_path = None
            self._cleanup_stale_wav()
            return
    ...
    def _cleanup_stale_wav(self) -> None:
        """清理 logs/ 下历史残留的 recent_*.wav 临时文件（每次转录最多尝试一次）。"""
        if getattr(self, "_stale_wav_cleaned", False):
            return
        self._stale_wav_cleaned = True
        try:
            for p in Path(self.log_dir).glob("recent_*.wav"):
                try:
                    p.unlink()
                except OSError:
                    pass
