...
class TrayApp:
    def __init__(self, toggle_callback, is_recording_fn, llm_config_path: str, log_dir: str):
        """
        ...
        menu = pystray.Menu(
            pystray.MenuItem("开始/停止录音  (F2)", self._on_toggle, default=True),
            pystray.Menu.SEPARATOR,
            pystray.MenuItem(
                lambda item: "LLM 润色：开" if self._llm_enabled else "LLM 润色：关",
                self._on_toggle_llm,
            ),
    ...
    def _on_toggle_llm(self, icon, item):
        try:
            ...
            cfg = {}
        cfg.setdefault("llm", {})
        cfg["llm"]["enabled"] = not self._llm_enabled
        try:
            with open(self._config_path, "w", encoding="utf-8") as f:
                json.dump(cfg, f, ensure_ascii=False, indent=2)
            self._llm_enabled = cfg["llm"]["enabled"]
            logger.info("LLM 润色开关已切换为 %s（重启程序生效）", self._llm_enabled)
        except Exception as exc:  # noqa: BLE001
    ...
    def _on_settings(self, icon, item):
        ...
        try:
            ...
            open_settings_dialog(self._config_path)
